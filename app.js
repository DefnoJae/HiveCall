(() => {
  ...
  function joinCall() {
    ...
    writePresence(g.id, { [S.user.id]: { name: S.user.name, color: S.user.color, mic: false, cam: false, share: false, joinedAt: now } });
    renderCall();
    bootMedia();
    startCallTimer();
    playChime();
  ...
  function callUser(uid) {
    ...
    try {
      const mc = peer.call(uid, c.stream);
      c.peers[uid] = mc;
  ...
  function onIncomingCall(mc) {
    ...
    const isShare = !!(roster[uid] && roster[uid].share && haveMain);
    if (isShare) {
